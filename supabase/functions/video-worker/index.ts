...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (metadata.gemini_file_name) {
      devLog('[video-worker] Polling Gemini File API for state:', metadata.gemini_file_name);
    ...
    devLog('[video-worker] Uploading video to Gemini File API:', job.asset_id);
