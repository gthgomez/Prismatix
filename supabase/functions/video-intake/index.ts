...
Deno.serve(async (req: Request) => {
  ...
  if (operation === 'init') {
    ...
    if (insertError) {
      devError('[video-intake] insert asset failed:', insertError);
    ...
    if (signedError || !signed?.signedUrl) {
      ...
      devError('[video-intake] createSignedUploadUrl failed:', signedError);
  ...
  if (listError) {
    devError('[video-intake] list storage object failed:', listError);
  ...
  if (updateError) {
    devError('[video-intake] update asset failed:', updateError);
