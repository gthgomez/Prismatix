...
Verification basis: Local workspace code scan on 2026-02-19 (not a production deployment verification).
...
- `debateProfile: 'general' | 'code'` — challenger set for debate (default: `'general'`)
...
- `X-Router-Model-Id`
- `X-Provider`
- `X-Model-Override` — value is `"debate:<profile>"` when debate is explicitly requested; `"auto"` or override value otherwise
- `X-Router-Rationale`
- `X-Complexity-Score`
...
Additive (debate mode, present in all responses, empty string when inactive):
- `X-Debate-Mode` — `"true"` | `"false"`
- `X-Debate-Profile` — `"general"` | `"code"` | `""`
- `X-Debate-Trigger` — `"explicit"` | `"auto"` | `"off"`
...
- Complexity >= 70 or totalTokens > 60000 -> `gemini-3-pro` (`images-complex`)
- Complexity <= 30 and totalTokens < 30000 -> `gemini-3-flash` (`images-fast`)
- Else -> `gemini-3-flash` (`images-default-flash`)

If request has no images:
- Code-heavy + complexity >= 45 + totalTokens < 90000 -> `sonnet-4.5` (`code-quality-priority`)
- Complexity >= 80 or totalTokens > 100000 -> `opus-4.5` (`high-complexity`)
...
- `gemini-3-flash` -> `gpt-5-mini` -> `sonnet-4.5`
...
rg -n "determineRoute|manual-override|images-complex|images-fast|code-quality-priority|high-complexity|ultra-low-latency|low-complexity|default-cost-optimized" supabase/functions/router/router_logic.ts
...
- `supabase/functions/video-worker/index.ts`: queue worker; currently writes deterministic placeholder artifacts.
...
- `Tests/Debate_test.ts` — 27 tests (eligibility, prompts, SSE contract, header regression)
