...
async function initVideoUpload(params: {
  ...
  devLog('[Storage][Video] Init upload:', {
...
export async function uploadVideoAttachment(
  ...
  devLog('[Storage][Video] Uploading to signed URL for:', file.name);
  const init = await initVideoUpload({ file, conversationId });
  await uploadViaSignedUrl(init.signedUploadUrl, file, onProgress);
  devLog('[Storage][Video] Signed upload complete. Finalizing asset:', init.assetId);
  await completeVideoUpload(init.assetId);
  devLog('[Storage][Video] Finalized asset:', init.assetId);
  return {
...
export async function uploadAttachment(
  ...
  if (!file.isImage || !file.imageData) {
    devLog('[Storage] Skipping non-image file');
    return null;
  }
  ...
  try {
    ...
    devLog('[Storage] Uploading to:', filePath);
    ...
    if (uploadError) {
      ...
      devError('[Storage] Upload error:', uploadError);
      return null;
    }
...
export async function checkBucketExists(): Promise<boolean> {
  try {
    ...
    if (error) {
      devWarn('[Storage] Bucket check failed:', error.message);
      return false;
    }

    return !!data;
  } catch (error) {
    devWarn('[Storage] Bucket check error:', error);
    return false;
  }
}
